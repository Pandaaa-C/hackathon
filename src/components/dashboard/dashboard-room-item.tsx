...
export default function DashboardRoomItem({roomData, joinRoom}: {roomData: IRoomInfo, joinRoom: (id: number, name: string) => void}) {
  return (
    <div className={cn("w-[265px] h-[175px] bg-black rounded p-2 flex flex-col")}>
      ...
      <div className={cn("w-full h-[15%] flex justify-center items-center")}>
        <button onClick={() => joinRoom(roomData.id, roomData.name)} className={cn("w-[100px] h-[90%] rounded bg-green-700 text-sm font-bold hover:bg-green-900 transition-colors disabled:cursor-not-allowed disabled:bg-gray-700")} disabled={roomData.currentPlayers >= roomData.maxPlayers}>Join room</button>
      </div>
    </div>
  )
}