...
import {IRoomInfo, IRoomUser} from "@/interfaces/room-info";
...
export default async function Home() {
    ...
    let roomsList = await db.rooms.findMany({});
    ...
    for (const room of roomsList) {
        rooms.push({
            ...
            currentPlayers: (JSON.parse(room.players) as IRoomUser[]).filter(x => x.accepted).length
        });
    };
    // const rooms = [
